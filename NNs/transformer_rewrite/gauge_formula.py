...
def candidate_signed(st64, D, H, dh, init, steps=400, lr=0.02, cond_pen=1e-4, log=None, sides=("qk", "av"), l1_mix=0.0, Ms=None, Ns=None, rot=False):
    ...
    rot=True: rotation-only (G = G_init . Cayley(S), orthogonal factor only -> the init's singular values and conditioning are kept)."""
    I = torch.eye(dh, dtype=init[0].dtype, device=init[0].device); cayley = lambda S: torch.linalg.solve(I + (S - S.transpose(-1, -2)), I - (S - S.transpose(-1, -2)))
    ...
    for step in range(steps + 1):
        ...
        if step == steps: break
        loss = tot + (0.0 if rot else cond_pen * sum((p ** 2).sum() + (torch.linalg.inv(p) ** 2).sum() for p in ps))
