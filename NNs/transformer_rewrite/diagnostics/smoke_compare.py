"""Golden-smoke comparison: python diagnostics/smoke_compare.py <reference.json> <new.json> [--lb_tol 1e-4]
...
one bisection grid step (1e-4 at the default grid) on at most one instance per set. Exit code 1 on failure.
...
for k in sorted(set(ref) & set(new)):
    ...
    r, n = ref[k], new[k]
    diffs = [(i, a, b) for i, (a, b) in enumerate(zip(r, n)) if abs(a - b) > 1.5e-4]
    steps = sum(1 for a, b in zip(r, n) if 1e-6 < abs(a - b) <= 1.5e-4)
    print(f"{k}: {len(r)} radii, {steps} off by one grid step, {len(diffs)} larger:", diffs[:5])
